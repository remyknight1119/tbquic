#ifndef QUIC_SRC_PACKET_H_
#define QUIC_SRC_PACKET_H_
...
#define TBQUIC_PACKET_HEADER_GET_TYPE(type) ((type & TBQUIC_PACKET_LPACKET_TYPE_MASK) >> 2)
#define TBQUIC_PACKET_HEADER_GET_PKE_NUM_LEN(type) ((type & TBQUIC_PACKET_LPACKET_PLT_NUM_LEN_MASK) >> 6)

typedef struct __attribute__ ((__packed__)) _tbquic_packet_header_t {
	uint8_t 	flags;
	uint32_t 	version;
